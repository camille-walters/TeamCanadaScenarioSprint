...
namespace PaintIn3D
{
	...
	public static partial class P3dCommon
	{
		...
		public static RenderTexture GetRenderTexture(RenderTextureDescriptor desc, bool sRGB)
		{
			...
			return CwRenderTextureManager.GetTemporary(desc, "P3dCommon GetRenderTexture");
		}
...
namespace PaintIn3D
{
	...
	public static partial class P3dCommon
	{
		...
		private static readonly int _P3dShapeTex     = Shader.PropertyToID("_P3dShapeTex");
		private static readonly int _P3dShapeChannel = Shader.PropertyToID("_P3dShapeChannel");

		public static void DrawShapeOutline(Texture shapeTexture, P3dChannel shapeChannel, Matrix4x4 shapeMatrix)
		{
			if (shapeTexture != null)
			{
				if (cachedShapeOutline == null)
				{
					cachedShapeOutline = CwHelper.CreateTempMaterial("Modifier Shape", "Hidden/Paint in 3D/ShapeOutline");
				}

				cachedShapeOutline.SetTexture(_P3dShapeTex, shapeTexture);
				cachedShapeOutline.SetVector(_P3dShapeChannel, IndexToVector((int)shapeChannel));
			...
			where T : MonoBehaviour
		{
			...
			var bestGuid   = default(string);
			var bestLength = default(long);

			foreach (var guid in guids)
			{
				var path = AssetDatabase.GUIDToAssetPath(guid);

				if (System.IO.Path.GetFileName(path) == typeof(T).Name + ".cs")
				{
					var info = new System.IO.FileInfo(path);

					if (info.Length > bestLength)
					{
						bestGuid   = guid;
			...
			where T : MonoBehaviour
		{
			...
			using (var stream = new System.IO.StreamReader(prefabPath))
			{
