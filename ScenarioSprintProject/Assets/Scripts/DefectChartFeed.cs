...
public class DefectChartFeed : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Graph.DataSource.ClearCategory("Throughput"); // clear the "Total Defects" category. this category is defined using the GraphChart inspector
    ...
    IEnumerator RedrawChart()
    {
        while (true)
        {
            Graph.DataSource.ClearCategory("Throughput");
            Graph.DataSource.StartBatch();
            var numPoints = analyticsData.totalDefectsList.Count;
            float x = 0f;
            for (int i = 0; i < numPoints; i++)  //add random points to the graph
            {
                Graph.DataSource.AddPointToCategoryRealtime("Throughput", x, analyticsData.totalDefectsList[i]); // each time we call AddPointToCategory 
                x += 3f;
